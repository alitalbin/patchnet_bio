...
class Trainer(BaseTrainer):
    ...
    def train_one_epoch(self, epoch):
        ...
        return self.train_loss_metric.avg, self.train_acc_metric.avg
            
    def train(self):
        for epoch in range(self.cfg['train']['num_epochs']):
            train_loss_metric, train_acc_metric = self.train_one_epoch(epoch)
            val_loss_metric, val_acc_metric = self.validate(epoch)
            # if epoch_acc > self.best_val_acc:
            #     self.best_val_acc = epoch_acc
            with open('training_metrics.csv', 'a') as myfile:
                writer = csv.writer(myfile, delimiter =',')        
                writer.writerow([epoch, train_loss_metric, train_acc_metric, val_loss_metric, val_acc_metric])
          
            self.save_model(epoch, val_loss_metric)
